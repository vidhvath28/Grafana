...
csv_file = os.path.join(output_folder, "azure-cost-per-account.csv")
with open(csv_file, "w", newline="") as file:
    writer = csv.writer(file)
    writer.writerow(["Subscription", "Total Cost (USD)", "Currency"])
    for row in query.rows:
        writer.writerow([row[0], row[1], "USD"])

print(f"✅ Azure cost per account saved: {csv_file}")
