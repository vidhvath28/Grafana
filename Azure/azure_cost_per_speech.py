...
# Function to get cost data for Azure Speech per resource
def get_speech_cost_data(subscription_id):
    ...
    query = {
        ...
        "dataset": {
            ...
            "grouping": [
                {"type": "Dimension", "name": "ResourceId"},  # Breakdown by resource
                {"type": "Dimension", "name": "ServiceName"}   # Filter for Speech Services
            ],
            "filter": {
                "dimensions": {"name": "ServiceName", "operator": "In", "values": ["Speech Services"]}
            }
        }
    }
...
def write_to_csv(all_data):
    csv_filename = "azure_speech_cost_per_resource.csv"
    with open(csv_filename, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["SubscriptionID", "PreTaxCost", "UsageDate", "ResourceId", "Currency"])
...
def main():
    ...
    for subscription_id in AZURE_SUBSCRIPTION_IDS:
        try:
            data = get_speech_cost_data(subscription_id)
